...
> The strategic-implementation skill now recognizes when a piece of work is one user-validateable outcome that spans several domains (commonly backend, the API/contract between them, and frontend) and cannot be honestly split into independently-checkable pieces. Instead of forcing that work into many small sub-deliverables that can't each be validated on their own, the skill treats it as a single "macro-deliverable" and builds its domains concurrently, validating them together as one unit. The operator watches the domains progress side-by-side and the whole outcome lands as one clean record of work, finishing in a fraction of the wall-clock a domain-by-domain build would take. Everything that *can* be cleanly decomposed still is — small and ordinary work runs exactly as before, with no added moving parts. When the underlying parallel capability is off, unavailable, or interrupted, the same macro-deliverable is simply built one domain at a time instead, with nothing lost.
...
A single cross-domain macro-deliverable is observed executing as one workflow in `/workflows` and lands as **one** atomic commit in measurably less wall-clock than building its domains one after another — while normal and small deliverables trigger no workflow at all, and the per-feature bookkeeping (one record per deliverable, consistent progress/deviation logs) stays identical to a sequential run. That macro-deliverable's validation is held to proving the integrated cross-domain outcome, and the validation approach it used is captured so a later similar feature recalls it instead of guessing.
