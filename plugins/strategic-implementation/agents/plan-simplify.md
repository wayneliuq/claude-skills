...
4. **Existing-capability opportunities (not just named symbols).** The plan reinvents something the repo already has. Check at the *capability/purpose* level, not only by name — for each capability a deliverable introduces (a dialog, a picker, a renderer/parser, a create/write path), semantic-search the repo for an existing implementation of that purpose. The worst re-implementations are *inline* (a hand-rolled `<div role=dialog>`, a native `<select>`, an inline state write) and have no new symbol to grep. If an existing implementation exists and the plan doesn't route through it, flag a **convergence miss** — a second write/create/render path into an existing capability is a smell even when it is not "longer" (convergence ≠ brevity). The deliverable's `Convergence audit` field should already record the search and the routing decision; flag it if absent.
