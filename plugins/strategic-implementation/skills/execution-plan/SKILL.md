...
6. **Validation-approach recall.** For deliverables that will be cross-domain (`Macro-deliverable`) or `Integration-risk class: a|b|c`, do a **read-only** `grep` over the N most-recent prior `docs/strategic-implementation/*/validation-log.md` for `## APPROACH` blocks whose `Domains:` / method match this brief's integration-risk class or domains. Surface any hits as **advisory** input to the Step 3 validation-method choice (reuse a proven pipeline instead of re-guessing). This is advisory only — it informs, never dictates, the method. Absent / no match → proceed silently (no error, no block).
